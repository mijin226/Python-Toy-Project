...
subway_file_path = "2021_월별 승하차인원.csv"
...
with open(temps_file_path, mode='r') as file:
    ...
    for row in reader:
        ...
        if row[0].startswith('21-'):                                   ## '21-'(2021년도)으로 시작하는 데이터 추출
...
with open(subway_file_path, mode='r') as file:
    ...
    for row in reader:
        for i in range(12):  # 1월부터 12월까지의 데이터 처리
            try:
                person_subway = int(row[4 + i])  ## 월별 승하차인원 데이터
...
## 그래프 제목 설정
plt.title('Monthly Average Temperature and Subway Passengers in 2021')
